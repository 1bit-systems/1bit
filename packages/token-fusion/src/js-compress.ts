...
import type { CompressionOptions, CompressionResult, CompressionMode, StageStats } from "./types.js";
...
function semanticDedup(text: string, ctype: string, mode: CompressionMode): StageResult {
  ...
  for (const block of blocks) {
    if (!block.trim()) { kept.push(block); continue; }
...
const ABBREVIATIONS: Record<string, string> = {
  ...
  about: "approx", approximately: "approx",
...
function abbrev(text: string, ctype: string, mode: CompressionMode): StageResult {
  ...
  for (const [word, abbr] of Object.entries(ABBREVIATIONS)) {
    ...
    result = result.replace(regex, () => { count++; return abbr; });
  }
